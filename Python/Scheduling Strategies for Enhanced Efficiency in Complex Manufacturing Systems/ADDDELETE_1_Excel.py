...
def launch_dashboard():
        ...
        children=[
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    html.Button('Get KPI', id='kpi-button', n_clicks=0, style=button_style),
            ...
            dcc.Tabs(id='tabs', value='tab-input', children=[
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {'label': 'Machine Capacity Schedule', 'value': 'Machine Capacity Schedule'},
    ...
    def start_KPI():
        global allocation_process
        # Print statements to debug
        print("Starting KPI_Status.py...")
       
        
        # Verifying if the file exists
        script_path = r'KPI_Status.py'
        if not os.path.exists(script_path):
            print(f"Script not found at path: {script_path}")
            return
        try:
            ...
            print("KPI_Status.py started successfully")
        except Exception as e:
            print(f"Error starting KPI_Status.py: {e}")
            raise
    ...
    def control_allocation_check(read_clicks, initialise_clicks, start_clicks, stop_clicks, reset_clicks, kpi_clicks, close_read_modal,
                                close_initialise_modal, close_start_modal, close_stop_modal, close_reset_modal, close_kpi_modal,
                                interval_disabled):
        ...
        if triggered_id == 'read-button' and read_clicks:
            ...
            interval_disabled = True
        elif triggered_id=="kpi-button" and kpi_clicks:
            kpi_modal_open=True
            modal_body="KPI Summary have been generated in Summary_excel.xlsx"
            start_KPI()
            interval_disabled=True


        elif triggered_id.startswith('close'):
            return interval_disabled, None, False, False, False, False, False, False, "", "", "","","",""
        
        return (interval_disabled, None,read_modal_open, initialise_modal_open, start_modal_open, stop_modal_open, reset_modal_open, kpi_modal_open,
                ...
                modal_body if kpi_modal_open else "")
        ...
        if selected_plot=="Gantt Chart":
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    ...
                    title_font=dict(size=20, color="black", family="Arial, sans-serif", weight="bold"),  # Make the y-axis title bold
                    tickfont=dict(family="Arial, sans-serif", size=15, color="black", weight="bold")  # Make the y-axis tick labels bold
                )
            )
        elif selected_plot=="Machine Capacity Schedule":
            # Assuming df is already defined and time columns are converted to datetime
            df['Start Time'] = pd.to_datetime(df['Start Time'])
            df['End Time'] = pd.to_datetime(df['End Time'])

            # Filter out any rows related to 'OutSrc'
            df_filtered = df[df['Machine Number'] != 'OutSrc']

            # Create a mapping of machine numbers to numeric values
            machine_mapping = {machine: idx for idx, machine in enumerate(df_filtered['Machine Number'].unique())}
            df_filtered['Machine Numeric'] = df_filtered['Machine Number'].map(machine_mapping)

            # Explicit color mapping for each product
            color_map = {
                'Product 1': 'blue',
                'Product 2': 'yellow',
                'Product 3': 'green',
            ...
            # Create the initial Gantt chart with explicit product colors
            fig = px.timeline(df_filtered, 
                            x_start="Start Time", x_end="End Time", 
                            y="Machine Numeric", 
                            color="Product Name",
                            hover_data=["Components"],
                            title="Machine Operations Gantt Chart with Component Details (Without OutSrc)",
                            category_orders={"Machine Numeric": df_filtered['Machine Numeric'].unique()},
                            color_discrete_map=color_map)  # Explicitly set product colors

            # Add traces for non-working hours and weekends to include in legend
            # Non-working hours (Black) with square marker in legend
            fig.add_trace(go.Scatter(
                x=[None], y=[None], 
                mode='markers',
                marker=dict(size=12, color='black', symbol='square'),  # Square marker
                showlegend=True,
                name='Non-working Hours'
            ))

            # Weekend (Pink) with square marker in legend
            fig.add_trace(go.Scatter(
                x=[None], y=[None], 
                mode='markers',
                marker=dict(size=12, color='red', symbol='square'),  # Square marker
                showlegend=True,
                name='Weekends'
            ))

            # Function to add non-working hours (black) as a single block across all machines
            def add_non_working_hours(fig, start_date, end_date, machine_count):
                current_date = start_date
                while current_date < end_date:
                    # Set the end of the working day and the start of the next day
                    end_of_day = current_date.replace(hour=17, minute=0, second=0)
                    next_day_start = (current_date + timedelta(days=1)).replace(hour=0, minute=0, second=0)
                    next_day_end = next_day_start.replace(hour=9, minute=0, second=0)

                    # If today is Monday (or the first working day after a weekend)
                    if current_date.weekday() == 0:  # Monday
                        # Non-working block starts at 00:00 on Monday
                        fig.add_shape(type="rect",
                                    x0=current_date.replace(hour=0, minute=0, second=0),  # Start at 00:00 on Monday
                                    x1=current_date.replace(hour=9, minute=0, second=0),  # End at 09:00 on Monday
                                    y0=-0.5,  # Cover all machines
                                    y1=machine_count - 0.5,
                                    fillcolor="black", opacity=0.8, line_width=0)
                    # If today is Friday, stop non-working hours at 00:00 on Saturday
                    elif current_date.weekday() == 4:  # Friday
                        # Non-working block from Friday 17:00 to Saturday 00:00
                        fig.add_shape(type="rect",
                                    x0=end_of_day,  # Start at 17:00 on Friday
                                    x1=current_date.replace(hour=23, minute=59, second=59),  # End at 00:00 on Saturday
                                    y0=-0.5,  # Cover all machines
                                    y1=machine_count - 0.5,
                                    fillcolor="black", opacity=0.8, line_width=0)
                    # Skip adding non-working day blocks if today is a weekend (Saturday or Sunday)
                    elif current_date.weekday() in [5, 6]:  # Saturday or Sunday
                        pass  # Skip weekends
                    else:
                        # Normal non-working hours extend from 17:00 to 09:00 the next day (only for weekdays)
                        fig.add_shape(type="rect",
                                    x0=end_of_day,  # Start at 17:00
                                    x1=next_day_end,  # End at 09:00 the next day
                                    y0=-0.5,  # Cover all machines
                                    y1=machine_count - 0.5,
                                    fillcolor="black", opacity=0.8, line_width=0)
            ...
            # Add weekend shading for the entire plot
            def add_weekend_shading(fig, start_date, end_date, machine_count):
                current_date = start_date
                while current_date <= end_date:
                    # Check if it's a weekend (Saturday or Sunday)
                    if current_date.weekday() == 5:  # Start on Saturday 00:00
                        weekend_start = current_date.replace(hour=0, minute=0, second=0)
                        weekend_end = (current_date + timedelta(days=2)).replace(hour=0, minute=0, second=0)  # End at 00:00 on Monday
                        fig.add_shape(type="rect",
                                    x0=weekend_start,
                                    x1=weekend_end,
                                    y0=-0.5,  # Cover all machines (from -0.5 to machine_count - 0.5)
                                    y1=machine_count - 0.5,
                                    fillcolor="red", opacity=1.0, line_width=0, layer="below")
                    
                    # Move to the next day
                    current_date += timedelta(days=1)

            # Define the time range of the entire chart
            start_date = df['Start Time'].min().replace(hour=0, minute=0, second=0)
            end_date = df['End Time'].max().replace(hour=23, minute=59, second=59)
            machine_count = len(machine_mapping)

            # Add non-working hours (17:00 - 09:00) in black as a single block (skipping weekends)
            add_non_working_hours(fig, start_date, end_date, machine_count)

            # Add weekend shading in pink for the whole graph (Saturday 00:00 to Monday 00:00)
            add_weekend_shading(fig, start_date, end_date, machine_count)
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    tickvals=list(machine_mapping.values()),
                    ticktext=list(machine_mapping.keys())
                ),
                legend_title="Legend",
